...
parser.add_argument('-g', '--gru4rec_model', metavar='GRFILE', type=str, default='gru4rec_tf', help='Name of the file containing the GRU4Rec class. Can be used to select different varaiants. (Default: gru4rec_tf)')                                                                                                                                                                                                                        
parser.add_argument('-d', '--device', metavar='D', type=str, default='GPU:0', help='Device used for computations (default: GPU:0). Format <type>:<id>, e.g. GPU:0 is the first GPU.')
...
if args.device.upper() == 'CPU':
...
elif args.device.upper().startswith('GPU'):
    device_type, device_id = args.device.split(':')
    ...
    raise RuntimeError('Device {} is not supported'.format(args.device))
